...
list<Process> mlfq(pqueue_arrival workload)
{
  ...
  const int timeQuantum[numLevels] = {1, 4, 3, 7}; 
  list<Process> complete;
...
list<Process> lifo(pqueue_arrival workload) {
  ...
  while (!workload.empty()) {
    Process p = workload.top();
    workload.pop();
    p.first_run = p.duration;  // Not yet executed
    p.completion = p.arrival + p.duration;  // Completion time is calculated

    complete.push_front(p);  // Add the process to the front of the completed list
  }
